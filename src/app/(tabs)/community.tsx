...
import { useCommunitySongsStore } from '../../store/useCommunitySongsStore';
...
export type SubScreenProps = { searchQuery: string };
...
function SongsTab({ searchQuery }: SubScreenProps) {
  const userProfile = useAuthStore((state) => state.userProfile);
  const songs = useCommunitySongsStore((s) => s.songs);
  const loading = useCommunitySongsStore((s) => s.songsLoading);
  const error = useCommunitySongsStore((s) => s.songsError);
...
export default function CommunityScreen() {
  ...
  const initializeFeedsListener = useFeedStore((s) => s.initializeFeedsListener);
  const initializeSongsListener = useCommunitySongsStore((s) => s.initializeSongsListener);

  useEffect(() => {
    const unsubFeeds = initializeFeedsListener();
    const unsubSongs = initializeSongsListener();
    return () => {
      unsubFeeds();
      unsubSongs();
    };
  }, [initializeFeedsListener, initializeSongsListener]);
